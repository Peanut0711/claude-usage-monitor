...
## 2. 폴링 실패 시 "이전 데이터 유지" (graceful degradation)
- 지금은 전송 오류 시 큰 "API error" 화면이 뜸. 대신 **마지막 정상 대시보드 유지**하고
  구석에 작은 경고 표시(점/아이콘 또는 status 문구 변경)만.
- 타임아웃 15s + 1회 재시도는 이미 적용됨(`Api.cpp`). 그 다음 단계.
- 구현 메모: `gLastUsage.ok`였던 마지막 값을 별도 보관, 새 폴링 실패 시 그걸로 다시 그림 + "stale" 표식.
...
## 5. 사용량 히스토리 그래프 (스파크라인)
- 폴링값(5h/7d 사용률)을 링버퍼에 모아 미니 꺾은선으로 추세 표시.
- IO12 페이지 순환에 "히스토리" 한 장 추가하는 형태로.
